...
for (const file of await fs.readdir("cards")) {
	...
	try {
		...
		if (apiCard.cardID !== card.cardId) {
			allSuccessful = false;
			console.error("\x1b[31m" + file + ": Card ID is incorrect! (should be " + apiCard.cardID + ")\x1b[0m\n");
		}
		if (!card.values.base.names.includes(apiCard.cardID)) {
			allSuccessful = false;
			console.error("\x1b[31m" + file + ": Card name is incorrect! (should be " + apiCard.cardID + ")\x1b[0m\n");
		}
		if (apiCard.level != card.values.base.level) {
			allSuccessful = false;
			console.error("\x1b[31m" + file + ": Level is incorrect! (should be " + apiCard.level + ")\x1b[0m\n");
		}
		if (!card.values.base.cardTypes.includes(apiCard.cardType)) {
			allSuccessful = false;
			console.error("\x1b[31m" + file + ": Card type is incorrect! (should be " + apiCard.cardType + ")\x1b[0m\n");
		}
		...
		if (!typesCorrect) {
			allSuccessful = false;
			console.error("\x1b[31m" + file + ": Types are incorrect! (should be " + apiCard.types.join(", ") + ")\x1b[0m\n");
		}
		if (card.deckLimit !== (apiCard.deckLimit === 50? Infinity : apiCard.deckLimit)) {
			allSuccessful = false;
			console.error("\x1b[31m" + file + ": Deck limit is incorrect! (should be " + (apiCard.deckLimit === 50? "any" : apiCard.deckLimit) + ")\x1b[0m\n");
		}
		if (card.values.base.abilities.length !== apiCard.effects.filter(effect => effect.type !== "rule").length) {
			allSuccessful = false;
			console.error("\x1b[31m" + file + ": Abilities are incorrect! (should have " + apiCard.effects.length + " in total)\x1b[0m\n");
			...
			if (abilityTypes.get(ability.constructor) !== apiAbility.type) {
				allSuccessful = false;
				console.error("\x1b[31m" + file + ": Ability #" + (i+1) + " is incorrect! (should be " + apiAbility.type + ")\x1b[0m\n");
			} else if (ability instanceof abilities.TriggerAbility) {
				if (ability.mandatory !== apiAbility.mandatory) {
					allSuccessful = false;
					console.error("\x1b[31m" + file + ": Ability #" + (i+1) + " is incorrect! (mandatory should be " + (apiAbility.mandatory? "yes" : "no") + ")\x1b[0m\n");
				}
			}
		}
		if (apiCard.cardType === "unit" && card.values.base.cardTypes.includes("unit")) {
			if (apiCard.attack !== card.values.base.attack) {
				allSuccessful = false;
				console.error("\x1b[31m" + file + ": Attack is incorrect! (should be " + apiCard.attack + ")\x1b[0m\n");
			}
			if (apiCard.defense !== card.values.base.defense) {
				allSuccessful = false;
				console.error("\x1b[31m" + file + ": Defense is incorrect! (should be " + apiCard.defense + ")\x1b[0m\n");
			}
		}
	} catch (e) {
		allSuccessful = false;
